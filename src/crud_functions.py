...
def get_partie_by_id(partie_id: int):
    return collection_tint_raw_data.find_one({"Partie": partie_id}, {"_id": 0})


def get_tours_by_partie_id(partie_id: int):
    partie = collection_tint_raw_data.find_one({"Partie": partie_id}, {"_id": 0})
    if not partie:
        return None
    return partie.get("Tours", [])


def get_tour_by_id(partie_id: int, tour_id: int):
    partie = collection_tint_raw_data.find_one({"Partie": partie_id}, {"_id": 0})
    if not partie:
        return None
